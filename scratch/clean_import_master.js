...
async function run() {
  ...
  for (let i = 2; i < rows.length; i++) {
    ...
    // Fixed indices for the perfectly aligned FAZENDA BELA VISTA FINAL - FINALIZADO.xlsm
    const rawId = row[1];
    const rawName = row[2];
    ...
    if (!id) {
      console.warn(`Row ${i + 1} has name "${nome}" but no ID. Skipping!`);
      continue;
    }

    let cpf = String(row[3] || '').trim().replace(/\D/g, '');
    let formattedCpf = '000.000.000-00';
    if (cpf && cpf.length === 11 && validateCPF(cpf)) {
      formattedCpf = cpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4');
    }

    const bancoRaw = String(row[4] || '').trim();
    const banco = intelligentMatchBank(bancoRaw);
    const agencia = String(row[5] || '').trim();
    const rawOp = String(row[6] || '').trim();
    const tipo_conta = normalizeOperation(rawOp);
    const conta = String(row[7] || '').trim();
  ...
  console.log(`Parsed ${parsedCollabs.length} collaborators from Excel. Uploading in batches...`);
  ...
  console.log(`\nImport completed successfully! ${successfulInserts} collaborators imported into Supabase with correct IDs.`);
}
